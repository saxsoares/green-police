...
| **Incêndios oficiais (BOI)** | ${celula(contexto.incendiosOficiais.fonte)} | ${celula(contexto.incendiosOficiais.confiabilidade === 'INDISPONIVEL' ? 'NÃO VERIFICADO — a consulta aos Boletins de Ocorrência de Incêndio não foi realizada.' : contexto.incendiosOficiais.valor.registrosEncontrados ? `${contexto.incendiosOficiais.valor.quantidade} boletim(ns) num raio de ${contexto.incendiosOficiais.valor.raioMetros} m: ${contexto.incendiosOficiais.valor.detalhes.map(b => `BOI ${b.numeroBoi} (${b.dataDeteccao}) ${b.caracterizacaoArea}`).join('; ')}.${contexto.incendiosOficiais.valor.possivelCorrespondenciaComOFato ? ` POSSÍVEL CORRESPONDÊNCIA com o fato: BOI ${contexto.incendiosOficiais.valor.possivelCorrespondenciaComOFato} — confirmar junto ao órgão gestor.` : ' Nenhum com data coincidente à do fato.'}` : `O órgão respondeu e não registrou Boletim de Ocorrência de Incêndio num raio de ${contexto.incendiosOficiais.valor.raioMetros} m. Ausência de BOI não é ausência de incêndio: o boletim depende de acionamento e lavratura.`)} | \`${contexto.incendiosOficiais.confiabilidade}\` |
