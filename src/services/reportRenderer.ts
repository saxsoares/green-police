...
|---|---|---|---|
| **Histórico de AIA** | ${celula(contexto.aia.fonte)} | ${celula(contexto.aia.confiabilidade === 'INDISPONIVEL' ? 'NÃO VERIFICADO — a consulta não foi realizada. A ausência de registro aqui NÃO atesta ausência de autuação anterior.' : contexto.aia.valor.historicoEncontrado ? `REINCIDÊNCIA IDENTIFICADA: ${contexto.aia.valor.detalhes.map(d => `${d.numeroAia} (${d.infracao})`).join('; ')}` : 'A base respondeu à consulta e não retornou AIA prévio no ponto.')} | \`${contexto.aia.confiabilidade}\` |
